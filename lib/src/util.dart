...
/// Parses provided messages to chat messages (with headers and spacers)
/// and returns them with a gallery.
List<Object> calculateGroupChatMessages(
  List<types.Message> messages,
  types.User user, {
  String Function(DateTime)? customDateHeaderText,
  DateFormat? dateFormat,
  ...
  for (var i = messages.length - 1; i >= 0; i--) {
    ...
    if (repliedMessage is types.ImageMessage) {
      if (kIsWeb) {
        if (repliedMessage.uri.startsWith('http') ||
            repliedMessage.uri.startsWith('blob')) {
          gallery.add(
            PreviewImage(id: repliedMessage.id, uri: repliedMessage.uri),
          );
        }
      } else {
        gallery
            .add(PreviewImage(id: repliedMessage.id, uri: repliedMessage.uri));
      }
    }
  }
